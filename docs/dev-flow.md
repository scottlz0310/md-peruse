...
4番目のファイル変更監視は、実装量が大きいため2つのPull Requestへ分ける。前半は`notify`のイベント写像、監視イベント用のパス相対化、debounce窓の時間管理であり、実際のWatcherを持たないため実OSイベントなしで検証できる。後半はWatcherのライフサイクル、監視スコープの採番と破棄、`DirectoryChanged`の生成、オーバーフローと停止の通知である。
