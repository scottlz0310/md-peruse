...
- 依存ライセンス一覧の生成手段を確定し、条文を取得できないパッケージがある場合にCIを失敗させる。
...
- [x] P1の未決事項のうち、実装前に確定が必要なものが解消している。残るP1（lowlightの言語allowlist）はPhase 4へ割り当て済みであり、実装の中で確定する。

## 6. Phase 4: 機能実装

### 着手順

Phase 4は6.1から6.3の順に、層ごとに着手する。最小経路を先に縦へ通す進め方は採らない。Phase 3が層ごとに型と規則を確定させており、同じ区切りで実装すれば本書の記述と着手順がずれないためである。世代管理と監視スコープの競合はRust側とFrontend側の双方に回帰テストを要するが、層をまたいで小刻みに進めるより、各層の中でまとめて固定するほうがテストの置き場所が定まる。

引き換えに、実機の画面で動くまでの期間は長くなる。IPC契約と描画の想定違いが統合の段階まで顕在化しないため、6.2の着手時にRust側のcommandを実際に呼ぶ結線を先に行い、契約の齟齬をそこで洗い出す。

6.1 Rust Coreは次の5単位へ分け、この順序で着手する。単位ごとにPull Requestを分ける。単位が大きくなる場合は、その中でさらに分けてよい。

1. ワークスペースとパス境界（7.1）
2. ディレクトリ走査（6.2、6.3）
3. ファイル読込（6.3）
4. ファイル変更監視（6.4、6.5）
5. custom image protocol（5.4、7.3）

4番目のファイル変更監視は、実装量が大きいため3つのPull Requestへ分ける。1つ目は`notify`のイベント写像、監視イベント用のパス相対化、debounce窓の時間管理。2つ目は実測に基づく設計の確定、`DirectoryChanged`の生成、窓ごとのイベント数による縮退。ここまでは実際のWatcherを持たないため、実OSイベントなしで検証できる。3つ目がWatcherのライフサイクル、ルートの親の監視、監視スコープの採番と破棄、Tauri eventの送出であり、実機での結線を要する。

パス境界を最初に置くのは、走査・読込・画像のいずれもがワークスペース相対パスの解決を前提とするためである。この単位はFrontendから観測できる変化を持たず、境界判定のモジュールとセキュリティ回帰テストだけを置く。ワークスペースの切り替えと破棄は、Watcherと探索キャッシュの破棄を伴うため（6.1）、状態を持つ2番目以降の単位で実装する。
...
- Windows固有のパス比較（大文字小文字、8.3形式の短い名前、コンポーネント境界）
