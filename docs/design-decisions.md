...
削除は窓を閉じるまで確定させない。同じ窓の中で置換やrename先として復活しうるためである。規則は `src-tauri/src/watch.rs` の `coalesce` を正本とし、6.4で実測した列を含めてテストで固定する。`notify` の型をそのまま扱わず自前の生イベントへ写像するのは、規則をプラットフォームと`notify`のバージョンから切り離して検証するためである。
...
規則は `src/state/tab-status.ts` の `beginLoad`、`applyLoadResult`、`applyFileChange` を正本とし、完了順を反転させた場合と、読込中に変更・renameを受理した場合をテストで固定する。
...
- 非アクティブタブはスコープID、パス、タイトル、スクロール位置、状態（`loaded` / `stale` / `deleted`）、読込世代を保持する。`stale` は外部でファイルが変更され再読込が必要になった状態を指す（6.5）。本アプリはRead-onlyであり、編集による未保存状態は存在しない。
- 同一文書を重複して開かない。
