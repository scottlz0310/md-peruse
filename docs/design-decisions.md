...
- 非アクティブタブはパス、タイトル、スクロール位置、dirty状態を保持する。ここでのdirtyは、外部でファイルが変更され再読込が必要になった状態を指す（6.4）。本アプリはRead-onlyであり、編集による未保存状態は存在しない。
- 同一文書を重複して開かない。
...
- 最近使ったフォルダー、最後のワークスペース復元、複数ファイル引数の扱い（9.2）
- 260文字を超えるパスへの対応方針
