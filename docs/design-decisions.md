...
**監視が画像の変更を検知しても、それだけでは再描画が起きない。** 監視の通知（`FileChange`）はタブ向けの変更をMarkdownに絞っており（6.5）、画像だけを書き換えた場合はFrontendへ何も届かない（atomic replaceでも親ディレクトリの `directoryChanged` だけである）。上で述べた「再描画されるのに古い画像が表示される状態が生じない」は成り立つが、画像の変更そのものを再描画の契機にする経路はまだない。Frontendの描画単位（どの文書がどの画像を参照しているか）の持ち方と併せてPhase 4-2で決める（[tasks.md](../tasks.md) の「検討待ち」）。
...
読込の実装の正本は `src-tauri/src/read.rs` とする。ファイルを開く共有モードは標準ライブラリの既定（`FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE`）とし、こちらからは絞らない。閲覧している間にエディタが保存できなくなる事態を避けるためである。逆に、他のプロセスが共有を許さずに開いているファイルは読めない。この共有違反は `FileLocked` として原因を示し、自動での再試行は行わない（12章。例外は6.5のatomic replace直後の再読込に限る）。
...
Markdown本文のリンクがNFDで書かれ、実ファイルがNFCである場合（macOS由来のリポジトリで起こりうる）は解決に失敗する。リンク解決の側でNFCとNFDの両方を試すかは[tasks.md](../tasks.md)の「検討待ち」へ積み、Phase 4-2で判断する。
...
- 同じ文書が既に開いていれば、そのタブへ切り替える（プレビューで開き直しても固定タブは固定のまま）。読込中の遷移先も同じ文書として扱う。見出しを指すリンクで切り替えたときは、そのタブで見出しへ移り、履歴に積む。
- 新しいタブはアクティブタブの右に加える。
- アクティブタブを閉じたときは右隣、なければ左隣をアクティブにする。
- 非アクティブタブは本文を持たないため（上記）、アクティブにするたびに読み直し、離れたときのスクロール位置へ戻す。離れたときの位置は、画面に出ていたタブの分だけ記録する（切替直後の画面には前のタブの本文が残っているため、その位置を切替先へ書き込まない）。
...
- Frontendとの受け渡しは `get_ui_settings_command`（`UiSettings`）と `update_ui_settings_command`（`UiSettingsUpdate`。指定した項目だけを変える）とする。最近使ったフォルダー、最後のワークスペースの復元、ウィンドウ配置は後続の単位で実装し、それまで `UiSettings` の最近使ったフォルダーは空で返す。
...
「パッケージ化した開発版・テスト版で送信しない」ことは、Phase 4で回帰テストとして固定する。
