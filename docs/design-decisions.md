...
`F3` は検索欄を開いていない間も奪い、検索欄を開く。WebView2標準の検索バーは `F3` でも開くためである。検索欄はプレビュー本文があるときだけ置く。本文がない状態の `Ctrl+F` はWebView2標準の検索バーを開きうるが、これはブラウザーアクセラレータキーの扱い（tasks.md 検討待ち）でまとめて決める。
