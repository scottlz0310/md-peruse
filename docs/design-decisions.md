...
そのため、タブごとに読込世代を持つ。読込を開始するたびに世代を進め、応答を受け取ったときに開始時の世代がタブの最新と一致する場合だけ反映する。一致しない応答は破棄する。規則は `src/state/tab-status.ts` の `beginLoad` と `applyLoadResult` を正本とし、A/Bの完了順を反転させた場合を含めてテストで固定する。
