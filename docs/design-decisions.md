...
陳腐化した応答は、Frontendが呼び出しごとにローカルの世代カウンタを進め、応答受信時に自分の世代が最新かを確認して破棄する。`await invoke()` は呼び出しと応答を対応付けるため、カウンタをFrontend側だけで保持すれば世代を判定できる。
...
世代カウンタはワークスペース単位ではなく、走査要求の系列として単調増加させる。ワークスペースを切り替えたときもカウンタを進めることで、旧ワークスペースの応答を同じ仕組みで破棄する。この振る舞いはPhase 4で、同一パスの再走査とワークスペース切替の競合をテストで固定する。
...
エラーは次の構造を持ち、Frontendでの分岐に文字列比較を使わない。
