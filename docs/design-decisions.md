...
持ち越すのは、猶予中の保留と同じパスを持つイベントすべてとする。保留を作ったイベントだけを残すと、同じ窓で受けた同じパスの変更が先に確定してしまう。持ち越したイベントが次の窓を形作り、静穏も上限もそのイベントが届いた時点から測り直す。持ち越しは保留ごとの猶予で切れるため、対の届かないrenameが持ち越され続けることはなく、溜まるイベントは直近 `DEBOUNCE_MS` の範囲に収まる。
