...
- メニューをネイティブ実装として確定し、構成とアクセラレータを `src-tauri/src/menu.rs` へ追加（[design-decisions.md](./docs/design-decisions.md) 10.1）。標準タイトルバーを使う方針、OSのアクセシビリティ機構との統合、キーボード操作（`Alt` アクセスキー、矢印、`Esc`、ポップアップのフォーカス管理）の実装コストで優位である
  ...
  - アクセラレータは `Ctrl+O`（フォルダーを開く）、`Ctrl+W`（タブを閉じる）、`Ctrl+B`（サイドバー）、`F5`（再読み込み）、`Ctrl+Plus` / `Ctrl+Minus` / `Ctrl+0`（文字サイズ）。重複がないことをテストで固定した。Tauriは重複を後勝ちで登録し失敗を返さないため、実行するまで気づけない
  - `Ctrl` + `+` / `-` / `0` をアプリへ割り当てるため、WebViewのズームホットキーを無効にする。有効なままだとWebView全体の拡大とプレビュー本文の拡大が同じキーで二重に起きる
