...
- custom image protocolのresource ID契約を追加。ワークスペース単位のソルトと相対パス・更新時刻・バイト数のHMACをIDとし、文書単位で一括発行する。内容が変わればIDが変わるため長期キャッシュを返せる（[design-decisions.md](./docs/design-decisions.md) 5.4）
