...
- ファイル変更監視の前半として、`notify` の導入、イベントの写像、debounce窓の時間管理を実装（[design-decisions.md](./docs/design-decisions.md) 6.4、6.5）。Watcherのライフサイクルと監視スコープの採番は後半で実装するため、この時点でFrontendから観測できる変化はない
  ...
  - **上限で閉じる例外は「保留」で表す。** 保留とは、その時点で窓を閉じると `fileRemoved` を確定させてしまう状態であり、同じ窓で作り直されていない `Remove` と、対の届いていない `Modify(Name(From))` の2つを指す。atomic replaceは `Remove` から始まるため、rename元がまだ届いていない削除も保留に含める。含めないと、上限の直前に届いた `Remove` が置換の途中でも削除として確定する
  - **保留の猶予は「最も古い保留が届いた時刻」から測る。** 静穏の起点に載せると、無関係なパスの更新が続く間ずっと窓が延び、対の届かないrename（監視範囲外への移動）と組み合わさったときに、削除も他ファイルの変更も通知できないままイベントが溜まり続ける。起点を最も古い保留に固定するのは、保留が次々に現れても猶予の起点が前へ動かないようにするためである。いずれにせよ窓は `MAX_WINDOW_MS + DEBOUNCE_MS` で閉じる
