...
- IPCの契約としてプロトコルバージョン・request ID・キャンセルを導入しないことを確定し、根拠を [design-decisions.md](./docs/design-decisions.md) 5.3 へ記録。陳腐化した走査応答の破棄はFrontendの世代カウンタで行い、走査の応答は対象ディレクトリを示す `ScanResult` として返す
