...
[dev-flow.md](./docs/dev-flow.md) はPhase 1（MSIX技術スパイク）をPhase 2（開発基盤）より先に置くが、実際の着手順は次のとおりとする。

1. Phase 0のリポジトリ整備
2. Phase 1の最小アプリ（スケルトン）配置
3. Phase 2のうちCI/CDと品質ガードレールの先行配置
4. Phase 1の残り（MSIX生成、WACK、実測）

スケルトンとCI/CDを先に立てることで、以降の変更をPull Requestとレビューサイクルへ乗せられる状態を早期に確保する。フェーズの定義自体は変更しない。
